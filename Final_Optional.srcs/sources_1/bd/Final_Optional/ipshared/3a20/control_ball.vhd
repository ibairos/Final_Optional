...
architecture functional of control_ball is
    ...
    signal vel_x : integer range 2 to 6 := 3;
    signal vel_y : integer range 2 to 6 := 3;
	...
	Begin
		if (clk_25'event and clk_25='1') then
            ...
            if ld_new = '1' and game_over_var = '0' then
                ...
                if (dir_y = '0') then
                    -- Rebote arriba
                    -- La velocidad x e y puede cambiar depende de donde pegue en la barra
                    if (new_sw_x >= bar_pos_x - 4 and
                            new_sw_x <= bar_pos_x + 32 and
                                (new_sw_y + 4 <= bar_pos_y and new_sw_y + 4 >= bar_pos_y - vel_y)) then
                        dir_y <= '1';
                        -- Ver donde exactamente ha pegado y cambiar la velocidad en consecuencia
