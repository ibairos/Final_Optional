...
architecture functional of control_bar is
    -- State definition:
    type state_control is (init, wait_b, send_req_0, send_req_3 , rcv_req_0, rcv_req_3, wait_vs_0, wait_vs_3, b0_pressed, b3_pressed, wait_erase, write_new, wait_write, send_done, vs_one);
    
    -- output of register state:
    signal st : state_control;
    
    -- Output of internal registers to store data:
    signal old_sw : std_logic_vector(9 downto 1) := "001110000";
    signal new_sw : std_logic_vector(9 downto 1) := "001110000";
