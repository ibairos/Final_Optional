                ...
                if (vel_y = '0') then
                    ...
                    if (new_sw_x >= bar_pos_x - 4 and
                            new_sw_x <= bar_pos_x + 32 and
                                (new_sw_y + 4 <= bar_pos_y and new_sw_y + 4 >= bar_pos_y - 4)) then
                         vel_y <= '1';
                    end if;
                    -- Rebote flanco izquierdo
                    if (new_sw_y + 4 >= bar_pos_y and
                            new_sw_y <= bar_pos_y + 4 and 
                                (new_sw_x + 4 <= bar_pos_x and new_sw_x + 4 >= bar_pos_x - 2)) then
                         vel_x <= '1';
                    end if;
                    -- Rebote flanco derecho
                    if (new_sw_y + 4 >= bar_pos_y and
                            new_sw_y <= bar_pos_y + 4 and 
                                (new_sw_x >= bar_pos_x + 32 and new_sw_x <= bar_pos_x + 32 + 2)) then
                         vel_x <= '0';
                    end if;
                end if;
			    -- Movimiento normal
